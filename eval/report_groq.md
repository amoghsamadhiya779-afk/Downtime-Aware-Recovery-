...
n=300 · generator=0.1.0 · n_treated=227 · n_holdout=73

## Primary KPI — incremental ₹ recovered per 1,000 at-risk (vs. holdout)

- **Incremental: ₹4343290.66 per 1,000** (95% CI [1288637.03, 6983084.72], 2000 bootstrap resamples)
- Gross recovered ₹/case — treated: ₹6653.00, holdout: ₹2309.71 (printed beside incremental deliberately — the gap is the argument, not the gross figure)
- Gross recovery rate — treated: 27.3%, holdout: 6.8%

## AI quality — diagnosis on the AMBIGUOUS subset only

- Ambiguous macro-F1: **0.260** (n=88). Overall accuracy across all cases is NOT reported — it is inflated by the CLEAN majority, which never reaches the model.
  - TRANSIENT_INFRA: P=0.83 R=0.13 F1=0.23 support=38
  - CUSTOMER_FIXABLE: P=0.28 R=0.95 F1=0.43 support=20
  - INSTRUMENT_INVALID: P=0.42 R=0.36 F1=0.38 support=14
  - TERMINAL: P=0.00 R=0.00 F1=0.00 support=16

## Secondary metrics

- Wasted-attempt rate: 5.9% (9/153 attempts landed on a case whose hidden true class is TERMINAL)
- Retries deferred past downtime end (rule DOWNTIME_DEFER fired): 0
- Cases abandoned early, zero attempts spent: 74
...
- Policy veto rate: **23.5%** (47/200 RETRY proposals denied) — target band [5%, 40%]: near 0 means the gate is decorative, near 100% means the model is useless
