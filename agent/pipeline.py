...
def ingest(conn: sqlite3.Connection, pf: PaymentFailure, seed: int, rules: Rules, now) -> Cohort:
    ...
    with log_context(case_id=pf.case_id, order_id=pf.order_id, attempt_no=pf.attempt_no):
        logger.log_event(
            "request.ingest.started",
...
def process_case(
    ...
    *,
    ...
    with log_context(case_id=case_id, order_id=pf.order_id, attempt_no=pf.attempt_no):
        logger.log_event(
        ...
        def _finalize(trace: DecisionTrace) -> DecisionTrace:
            ...
            total_latency_ms = (time.perf_counter() - t_case_start) * 1000
            logger.log_event(
                ...
                policy_decision=trace.verdict.decision.value,
                execution_result=exec_res,
                action_succeeded=trace.action_succeeded,
            )
            return trace

        t_triage = time.perf_counter()
        tr = run_triage(pf.error.reason)
        triage_latency_ms = (time.perf_counter() - t_triage) * 1000
        logger.log_event(
            "agent.triage.completed",
            latency_ms=triage_latency_ms,
            triage_class=tr.recoverability.value,
            is_ambiguous=tr.is_ambiguous,
            matched=tr.matched,
        )
        ...
        if tr.is_ambiguous:
            logger.log_event("agent.diagnosis.started", reason=pf.error.reason, ambiguous=True)
            inp = DiagnosisInput(
                method=pf.method,
                error=pf.error,
                amount_paise=pf.amount_paise,
        ...
        diag_latency_ms = (time.perf_counter() - t_diag) * 1000
        logger.log_event(
            "agent.diagnosis.completed",
            latency_ms=diag_latency_ms,
            recoverability=proposal.recoverability.value,
            confidence=proposal.confidence,
            proposed_action=proposal.proposed_action.value,
            proposed_delay_minutes=proposal.proposed_delay_minutes,
            fallback_tier=proposal.fallback_tier,
        )
        ...
        policy_latency_ms = (time.perf_counter() - t_policy) * 1000
        logger.log_event(
            "policy.evaluate.completed",
        ...
        logger.log_event(
            ...
            action=verdict.action.value,
            execute_at=verdict.execute_at.isoformat() if verdict.execute_at else None,
        )
        transition(conn, case_id, "EXECUTING")
        t_exec = time.perf_counter()
        try:
            result = executor.execute(verdict)
            exec_latency_ms = (time.perf_counter() - t_exec) * 1000
            logger.log_event(
                "execution.completed",
